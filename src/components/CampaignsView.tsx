...
export const CampaignsView: React.FC<CampaignsViewProps> = ({
  ...
  const campaigns: CatalogItem[] = settings.catalogCampaigns || [];
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    if (!cleanName) {
      setFormError('Informe o nome da campanha publicitária.');
      return;
    }
    ...
    if (isDuplicate) {
      setFormError('Já existe uma campanha com este nome cadastrada.');
      return;
    }
    ...
    if (editingItem) {
      ...
      onNotify(`Campanha "${cleanName}" atualizada com sucesso.`);
    } else {
      const newCamp: CatalogItem = {
        id: `camp-${Date.now()}-${Math.floor(Math.random() * 1000)}`,
        name: cleanName,
        active: formActive,
      };
      updated = [newCamp, ...campaigns];
      onNotify(`Nova campanha "${cleanName}" criada com sucesso.`);
    }

    onSaveSettings({
      ...settings,
      catalogCampaigns: updated,
      campaigns: updated.filter((c) => c.active).map((c) => c.name),
    });

    setIsModalOpen(false);
  };

  const handleToggleActive = (c: CatalogItem) => {
    ...
    onSaveSettings({
      ...settings,
      catalogCampaigns: updated,
      campaigns: updated.filter((item) => item.active).map((item) => item.name),
    });
    onNotify(
      c.active
        ? `Campanha "${c.name}" foi pausada/desativada. Atendimentos passados permanecem intactos.`
        : `Campanha "${c.name}" foi reativada para seleção no atendimento.`
    );
  };
  ...
  const filteredCampaigns = campaigns.filter((c) => {
    ...
    if (searchTerm.trim() && !c.name.toLowerCase().includes(searchTerm.toLowerCase())) return false;
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 sm:p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Gerenciamento dinâmico de campanhas publicitárias (Instagram Ads, Saldões, Feirões, etc.)
          </p>
        </div>
      ...
      {/* Regra de Preservação do Histórico */}
      <div className="bg-amber-50/70 border border-amber-200 rounded-xl p-4 flex items-start gap-3 text-xs text-amber-900">
        ...
        <div>
          <p className="font-bold">Preservação do Histórico de Campanhas</p>
          <p className="text-amber-800 mt-0.5 leading-relaxed">
            Ao encerrar uma campanha (desativar) ou renomeá-la, todos os relatórios e atendimentos históricos continuam vinculados com o nome original da época em que ocorreram.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs flex flex-col sm:flex-row sm:items-center gap-3 justify-between">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Pesquisar campanha por nome (ex: Sofá, Saldão, Black Friday)..."
            value={searchTerm}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {filteredCampaigns.length === 0 ? (
          <div className="col-span-full py-12 text-center text-slate-400 bg-white rounded-2xl border border-slate-200">
            Nenhuma campanha encontrada com os termos pesquisados.
          </div>
        ) : (
          filteredCampaigns.map((c) => (
            <div
              ...
              className={`p-4 bg-white rounded-2xl border transition-all ${
                c.active ? 'border-slate-200 shadow-xs' : 'border-slate-200 bg-slate-50/60 opacity-65'
              }`}
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-start gap-2.5">
                  <div className={`p-2 rounded-xl mt-0.5 ${c.active ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-100 text-slate-400'}`}>
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button
                    ...
                    title={c.active ? 'Pausar Campanha' : 'Ativar Campanha'}
                    className={`p-1.5 rounded-lg transition-colors ${
                      c.active ? 'text-rose-500 hover:bg-rose-50' : 'text-emerald-600 hover:bg-emerald-50'
                    }`}
                  >
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden border border-slate-200 animate-scaleIn">
            <div className="flex items-center justify-between px-6 py-4 bg-slate-50 border-b border-slate-200">
              <h2 className="text-base font-bold text-slate-800">
                {editingItem ? 'Editar Campanha' : 'Nova Campanha Promocional'}
              </h2>
              <button
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4 text-xs">
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="Ex: Campanha Sofá, Feirão de Cozinhas, Black Friday..."
                  value={formName}
              ...
              <div>
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  ...
                  <span className="font-semibold text-slate-700">
                    Campanha Ativa (disponível para seleção no atendimento)
                  </span>
                </label>
              </div>
              ...
              <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-200">
                ...
                >
                  {editingItem ? 'Salvar Alterações' : 'Criar Campanha'}
                </button>
              </div>
            </form>
          </div>
